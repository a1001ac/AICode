...
/** 此处后端没有提供注释 POST /user/retrievePassword */
export async function userRetrievePassword(
  body: API.UserRetrievePasswordRequest,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean>('/user/retrievePassword', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
/** 此处后端没有提供注释 POST /user/sendResetCode */
export async function sendResetPasswordCode(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.sendResetPasswordCodeParams,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean>('/user/sendResetCode', {
    method: 'POST',
    params: {
      ...params,
    },
...
/** 此处后端没有提供注释 POST /user/update/role */
export async function updateUserRole(
  body: API.UserUpdateRoleRequest,
  options?: { [key: string]: any }
) {
